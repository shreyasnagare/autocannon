...
function requestBuilder (defaults) {
  ...
  return function buildRequest (reqData) {
    ...
    if (bodyBuf && bodyBuf.length > 0) {
      let idCount = reqData.idReplacement
        ? (bodyBuf.toString().match(/\[<id>\]/g) || []).length
        : 0
      let id_count = reqData.idReplacement
        ? (bodyBuf.toString().match(/~id~/g) || []).length
        : 0
      let tmCount = reqData.idReplacement
        ? (bodyBuf.toString().match(/~tm~/g) || []).length
        : 0
      headers['Content-Length'] = `${bodyBuf.length + (idCount * 4) + (id_count * 6) + (tmCount * 10)}`  
    }
